const EHRChain = require('./ehrChain');
const WebSocket = require('ws');

const EHRNode = function(port){
    let ehrSockets = [];
    let ehrServer;
    let _port = port
    let chain = new EHRChain();
    ...
    function init(){
        ...
        ehrServer = new WebSocket.Server({ port: _port });
		
        ehrServer.on('connection', (connection) => {
    ...
    const broadcastMessage = (event, message) => {
        ehrSockets.forEach(node => node.send(JSON.stringify({ event, message})))
    }

    const closeConnection = (connection) => {
        console.log('closing connection');
        ehrSockets.splice(ehrSockets.indexOf(connection),1);
    }
